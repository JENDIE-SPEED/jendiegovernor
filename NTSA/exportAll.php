...
require_once('TCPDF-master/tcpdf_import.php');
      ...
      $obj_pdf->AddPage();
        ...
        $style = array(
            ...
            'bgcolor' => false
                ...
                $obj_pdf->Text(1,65,'FITTING AGENT');
                $obj_pdf->Text(80,64,$dealer);
                $obj_pdf->Text(1,70,'BUSINESS REG NUMBER');
                $obj_pdf->Text(80,69,'CPR/125682');
                $obj_pdf->Text(1,75,'FITTING AGENT EMAIL');
                $obj_pdf->Text(80,74,'info@jendiespeedgovernors.com');
                $obj_pdf->Text(1,80,'FITTING AGENT ID');
                $obj_pdf->Text(80,79,'JDN-001');
                $obj_pdf->Text(1,85,'FITTING AGENT PHONE');
                $obj_pdf->Text(80,84,'0720522544');
                $obj_pdf->Text(1,90,'N & L OF FITTING');
                $obj_pdf->Text(80,89,$location);
 ...
 {  
    ...
    $serial=$_GET['id'];
    $serial = '0'.$serial;
     $sql = "select * from data1 where vehicle='$serial' and date = '$new_date' order by id desc";  
    }else if(isset($_GET['1tp'])){ 
    $serial=$_GET['id'];
    $serial = '0'.$serial;
    $sql = "select * from data1 where vehicle='$serial' AND velocity!='0KM/H' order by id desc limit 720";
	}else if(isset($_GET['6tp'])){ 
    $serial=$_GET['id'];
    $serial = '0'.$serial;
    $sql = "select * from data1 where vehicle='$serial' AND velocity!='0KM/H' order by id desc limit 1000";
	}else if(isset($_GET['72tp'])){ 
    $serial=$_GET['id'];
    $serial = '0'.$serial;
    $sql = "select * from data1 where vehicle='$serial' AND  velocity!='0KM/H' order by id desc limit 122";
	}else if(isset($_GET['vol'])){ 
    $serial=$_GET['vol'];
    $serial = '0'.$serial;
    $sql = "select * from data1 where vehicle='$serial' AND velocity >'81KM/H' order by id desc limit 100";
    }
      ...
      $lati= $row['latitude'] ;
       ...
       $violations = "";
      $speed_signal = $row['speedDisConn'];
      $power = $row['poweralarm'];
      if($power ==1){
        $violations = "Power Disconnected";
      }
	  if($speed_signal ==1){
        //$violations = $violations." "."Speed Signal Disconnected";
      }
      $overspeed = $row['velocity'];
      if(strpos($overspeed, 'KM/H') == false){
        
      } else{
        $new_speed = substr($overspeed, 0, -3);
      ...
      if(isset($_GET['vol'])){
	  if($power == 1 || $speed_signal == 1 || $new_speed >80){
		  $i++;  
      $output .= '<tr>  <td>'.$i.'</td>
                        <td>'.$row['date'].'</td>
                        <td>'.$row['time'].'</td>
